...
function filtered(){const q=search.value.trim().toLowerCase(),s=statusFilter.value;return listings.filter(x=>(s==='all'||x.status===s)&&(!q||[x.id,x.type,x.city,x.district].join(' ').toLowerCase().includes(q)))}
function renderRows(){const data=filtered();document.getElementById('propertyCount').textContent=`${data.length} عقار`;document.getElementById('adminEmpty').hidden=!!data.length;rows.innerHTML=data.map(x=>`<tr><td><div class="property-cell"><img src="${x.img}" alt=""><span><strong>${x.type}</strong><small>${x.deal}</small></span></div></td><td>#${x.id}</td><td>${x.district}<br><small>${x.city}</small></td><td><strong>${money(x.price)} ر.س</strong><br><small>${x.period||''}</small></td><td><span class="status ${statusClass(x.status)}">${x.status}</span></td><td><span class="feature-dot ${x.featured?'on':''}" title="${x.featured?'مميز':'غير مميز'}"></span></td><td><div class="row-actions"><button data-edit="${x.id}">تعديل</button><button data-toggle="${x.id}">${x.status==='غير متاح'?'تفعيل':'إيقاف'}</button><button class="danger" data-delete="${x.id}">حذف</button></div></td></tr>`).join('');bindActions()}
function render(){renderStats();renderRows()}
function openDrawer(item=null){form.reset();uploadedData='';form.editingId.value=item?item.id:'';document.getElementById('drawerTitle').textContent=item?'تعديل العقار':'إضافة عقار';if(item){for(const [k,v] of Object.entries(item)){if(k==='features'||k==='featured'||k==='img'||k==='id')continue;if(form.elements[k])form.elements[k].value=v??''}form.elements.id.value=item.id;form.elements.features.value=(item.features||[]).join('\n');form.elements.img.value=item.img||'';form.elements.featured.checked=!!item.featured;setPreview(item.img)}else{form.elements.id.value=getNextListingId();form.elements.status.value='متاح';form.elements.featured.checked=true;setPreview('')}drawer.classList.add('open');drawer.setAttribute('aria-hidden','false');document.body.style.overflow='hidden'}
